...
Carrillo C., McKay S.K., and Swannack T.M. 2020. Ecological Model Development: Toolkit for interActive Modeling (TAM). ERDC TN-EMRRP-??.  U.S. Army Engineer Research and Development Center, Vicksburg, Mississippi.
...
#### *F.1. Existing condition assessment over multiple sites using multiple USFWS models*  
...
for(i in 1:nPW){
  SI.PW[i, ] <- SIcalc(PW.model, as.character(input.PW[i, ]))
}
...
for(i in 1:nBO){
  SI.BO[i, ] <- SIcalc(BO.model, as.character(input.BO[i, ]))
}
...
for(i in 1:nGS){
  row_i <- c(
    as.character(GS$SIV1[i]),
    GS$SIV2[i],
    as.character(GS$SIV3[i]),
    as.character(GS$SIV4[i]),
    as.character(GS$SIV5[i])
  )
  SI.GS[i, ] <- SIcalc(GS.model, row_i)
}
...
for(i in 1:nMNK){
  ...
  SI.MNK[i, ] <- SIcalc(MNK.model, row_i)
}
...
#### *F.2. User-specified Model: Tidal Habitats and Resilience Evaluation Model (THREM) (Reviewed and approved by the ECO-PCX)*  

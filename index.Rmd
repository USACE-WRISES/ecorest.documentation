...
# Updated local functions from Git package-update project- Darixa
...
#Check what version of SIcalc is rendering:
#cat("SIcalc environment:", environmentName(environment(SIcalc)), "\n")
#print(SIcalc)
...
tryCatch({
  if (any(duplicated(timevec))) {
    dup_times <- unique(timevec[duplicated(timevec)])
    stop(
      paste0("Duplicate time intervals detected: ", paste(dup_times, collapse = ", "), "."),
      call. = FALSE
    )
  }
}, error = function(e) {
...
3) `HSIeqtn()` combines suitability index values into a single habitat suitability index using the model-specific equation stored in HSImetadata.These equations correspond to existing USFWS “blue book” HSI models that have been approved for USACE use.
...
5) `HSIarimean()` takes the arithmetic mean of user inputs, which allows higher suitability scores to compensate for lower scores. Although this function provides general methods for aggregating suitability index values, USACE applications should use only models that have received appropriate Eco-PCX review and approval.
```{r}
HSIarimean(c(0.5, 0.3, 0.7))

```

6) `HSIwarimean()` takes the weighted arithmetic mean of user inputs according to user-provided weights that sum to one. This allows users to emphasize the importance of certain environmental conditions for overall habitat suitability.Although this function provides general methods for aggregating suitability index values, USACE applications should use only models that have received appropriate Eco-PCX review and approval.
```{r}
HSIwarimean(x = c(0.3, 0.7, 1), w = c(0.3, 0.3, 0.4))
```

7) `HSIgeomean()` takes the geometric mean of user inputs, which ensures that one completely unsuitable environmental condition will result in unsuitable habitat for a species.Although this function provides general methods for aggregating suitability index values, USACE applications should use only models that have received appropriate Eco-PCX review and approval.
```{r}
HSIgeomean(c(0, 0.2, 1))
```
8) `HSImin()` takes a limiting factor approach, such that the overall HSI score is equal to the lowest suitability index.Although this function provides general methods for aggregating suitability index values, USACE applications should use only models that have received appropriate Eco-PCX review and approval.
```{r}
...
Murray, Elizabeth O., Darixa D. Hernandez-Abrams, and Todd M. Swannack. 2021. Development of Tidal Habitats and Resilience Evaluation Model (THREM) for Evaluation of Ecosystem Alternatives. Internal model documentation. U.S. Army Corps of Engineers.
...
* *CEICA*: Cost-Effective Incremental Cost Analysis
...
* *THREM*: Tidal Habitats and Resilience Evaluation Model
...
***Step 1:*** Load `ecorest`, model objects, and example field inputs for all species.
...
![Figure E.1.1. Pileated Woodpecker (eastern range) suitability curves.](PW.jpeg)

![Figure E.1.2. Barred Owl suitability curves.](BO.jpeg)

![Figure E.1.3. Gray Squirrel suitability curves.](GS.jpeg)

![Figure E.1.4. Mink (lacustrine model) suitability curves.](MNK.jpeg)
...
for(i in 1:nPW){
  SI.PW[i, ] <- suppressWarnings(
    SIcalc(PW.model, input.PW[i, ])
  )
}
...
for(i in 1:nBO){
  SI.BO[i, ] <- suppressWarnings(
    SIcalc(BO.model, input.BO[i, ])
  )
}
...
for(i in 1:nGS){
  SI.GS[i, ] <- suppressWarnings(
    SIcalc(GS.model, input.GS[i, ])
  )
}
...
for(i in 1:nMNK){
  SI.MNK[i, ] <- suppressWarnings(
    SIcalc(MNK.model, input.MNK[i, ])
  )
}
...
knitr::kable(
  SI.summary.E1,
  caption = "Table E.1.1. Suitability index values for all species across hypothetical sites."
)
```
...
knitr::kable(
  marsh.field,
  caption = "Table E.2.1.THREM field input data."
)
...
knitr::kable(
  marsh.model.view,
  caption = "Table E.2.2.THREM user-specified suitability curve input data."
)
```
...
```
![Figure E.2.1. Tidal Habitats and Resilience Evaluation Model](marsh.plot.jpeg)
...
knitr::kable(
  SI.marsh.out,
  caption = "Table E.2.3.: Suitability index values calculated using SIcalc()."
)
```
...
![Figure E.2.2. Tidal Habitats and Resilience Evaluation Model HSI Equation](THREM_model_equation.jpg)
```{r, echo=FALSE}
# Final output table
...
knitr::kable(
  final.marsh,
  caption = "Table E.2.4. Example output for a user-specified Tidal Habitats and Resilience Evaluation Model."
)
...
knitr::kable(
  Table.E2.inputs,
  caption = "Table E.3.1. Annualized habitat benefits and project costs for a hypothetical single-site restoration analysis."
)
```
...
knitr::kable(
  Table.E3,
  caption = "Table E.3.2. Incremental cost analysis summary for hypothetical single-site habitat restoration alternatives."
)
...
#### *E.4. Multi-site restoration prioritization using habitat units and CEICA*
...
knitr::kable(
  HU.summary.E3,
  caption = "Table E.4.1. Habitat units for each taxon across hypothetical restoration sites."
)
...
knitr::kable(
  Benefit.summary.E3,
  caption = "Table E.4.2. Total habitat units and project costs used in the CEICA."
)
...
knitr::kable(
  Table.E3,
  caption = "Table E.4.3. Incremental cost analysis summary for multi-site restoration prioritization."
)
